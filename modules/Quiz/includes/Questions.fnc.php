...
function QuizGetQuestionsForm( $title, $RET, $extra_category_fields = [], $type_options = null )
{
	...
	{
		$delete_URL = "Modules.php?modname=" . $_REQUEST['modname'] . '&modfunc=delete&category_id=' . $category_id . '&id=' . $id;
...
function QuizGetAddQuestionToQuizForm( $RET )
{
	...
	$form = '<form action="' . URLEscape(
			'Modules.php?modname=Quiz/Quizzes.php&modfunc=save&category_id='
		) . '" method="POST" id="quiz-add-question-form">';
	...
	if ( AllowUse( 'Quiz/Quizzes.php' ) )
	{
		...
		{
			$header .= '<li><a href="' . URLEscape(
					'Modules.php?modname=Quiz/Quizzes.php&category_id=' . $question_quiz[1]['QUIZ_ID']
				) . '">' . $question_quiz[1]['TITLE'] . '</a></li>';
		}
	...
	else
	{
		...
		$header .= '<td>' . SelectInput(
			...
			$cp_quiz_options,
			'N/A',
			'required autocomplete="off" class="onchange-quiz-select"'
		) . '</td>';
	...
	$header .= '<script src="modules/Quiz/js/QuizGetAddQuestionToQuizForm.js?v=10.8"></script>';
...
if ( ! function_exists( 'QuizQuestionTypeInputExtra' ) )
{
	...
	function QuizQuestionTypeInputExtra( $answer_input_id )
	{
		...
		$extra = 'autocomplete="off" class="onchange-quiz-question-type-input"';

		$type_options_answer_input = QuizQuestionGetAnswerInput();

		// Print our JS code directly, do not return.
		?>
		<input type="hidden" disabled id="quiz_answer_input_id"
			value="<?php echo AttrEscape( $answer_input_id ); ?>" />
		<input type="hidden" disabled id="quiz_question_type_options"
			value="<?php echo AttrEscape( json_encode( $type_options_answer_input ) ); ?>" />
		<script src="modules/Quiz/js/QuizQuestionTypeInputExtra.js?v=11.3"></script>
		<?php
