 ...
 * & Submission form.
 *
 * @since 11.1 CSS use .legend-gray.size-1 for details label
 *
...
function StudentQuizSubmissionOutput( $quiz_id, $student_id = 0 )
{
	...
	// Past due, in red.
	$due_date = $quiz['DUE_DATE'] ? MakeQuizDueDate( $quiz['DUE_DATE'] ) : _( 'N/A' );

	$assigned_date = $quiz['ASSIGNED_DATE'] ? ProperDate( $quiz['ASSIGNED_DATE'] ) : _( 'N/A' );
	...
	// Course - Teacher.
	DrawHeader(
		'<span class="legend-gray size-1">' . _( 'Course Title' ) . '</span> ' . $quiz['COURSE_TITLE'],
		'<span class="legend-gray size-1">' . _( 'Teacher' ) . '</span> ' . GetTeacher( $quiz['STAFF_ID'] )
	);
	...
	if ( $quiz['TITLE'] !== $quiz['ASSIGNMENT_TITLE'] )
	{
		// Title - Type.
		DrawHeader(
			'<span class="legend-gray size-1">' . _( 'Title' ) . '</span> ' . $quiz['TITLE'],
			'<span class="legend-gray size-1">' . _( 'Assignment' ) . '</span> ' . $type_color . $quiz['ASSIGNMENT_TITLE']
		);

		// Points.
		DrawHeader(
			'<span class="legend-gray size-1">' . _( 'Points' ) . '</span> ' . $quiz['POINTS']
		);
	}
	else
	{
	...
	{
		// Description.
		DrawHeader( '<span class="legend-gray size-1">' . _( 'Description' ) . '</span><br />
	...
	if ( $quiz['ASSIGNMENT_FILE'] )
	{
		...
		DrawHeader(
			'<span class="legend-gray size-1">' . _( 'File' ) . '</span> ' . GetAssignmentFileLink( $quiz['ASSIGNMENT_FILE'] )
		);
	}
...
function QuizStudentQuizGradedAJAXUpdate( $quiz_id, $student_id, $total_points )
{
	...
	// Note: hidden input values used by JS below.
	// Note 2: We use CSS class instead of ID to allow for various calls without ID conflict.
	?>
	<input type="hidden" disabled class="points-input-id" value="<?php echo AttrEscape( $points_input_id ); ?>" />
	<input type="hidden" disabled class="quiz-points-total-id" value="<?php echo AttrEscape( $points_total_id ); ?>" />
	<input type="hidden" disabled class="quiz-total-points" value="<?php echo AttrEscape( $total_points ); ?>" />
	<script src="modules/Quiz/js/QuizStudentQuizGradedAJAXUpdate.js?v=10.8"></script>
	<?php
