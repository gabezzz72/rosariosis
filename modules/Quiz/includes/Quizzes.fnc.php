...
function QuizGetAssignmentLinkHeader( $quiz_id )
{
	...
	DrawHeader( '<a href="' . URLEscape(
			'Modules.php?modname=Grades/Assignments.php&assignment_id=' . $assignment['ASSIGNMENT_ID']
		) . '"><b>' . _( 'Assignment' ) . '</b></a>' );
}
...
function QuizGetPreviewLinkHeader( $quiz_id )
{
	...
	DrawHeader( '<a href="' . URLEscape(
			'Modules.php?modname=Quiz/Quizzes.php&category_id=' . $quiz_id . '&modfunc=preview'
		) . '"><b>' . _( 'Preview' ) . '</b></a>' );
}
...
function QuizGetGradesLinkHeader( $quiz_id )
{
	...
	DrawHeader( '<a href="' . URLEscape(
			'Modules.php?modname=Grades/Grades.php&type_id=' .
...
function QuizGetTeacherLinkHeader( $quiz_id )
{
	...
	// Is Quiz owner?
	$is_quiz_owner = ( User( 'STAFF_ID' ) === $quiz['STAFF_ID'] );
	...
	DrawHeader(
		_( 'Teacher' ) . ': <a href="' . URLEscape(
			'Modules.php?modname=Users/User.php&staff_id=' . $quiz['STAFF_ID']
		) . '">' . GetTeacher( $quiz['STAFF_ID'] ) . '</a>',
...
function QuizGetQuizzesForm( $title, $RET, $extra_category_fields = [], $type_options = null )
{
	...
	$form = '<form action="' . URLEscape( $action ) . '" method="POST">';
		...
		&& $id )
	{
		...
		$delete_URL = "Modules.php?modname=" . $_REQUEST['modname'] . '&modfunc=delete&category_id=' . $id;

		$onclick_link = 'ajaxLink(' . json_encode( URLEscape( $delete_URL ) ) . ');';
	...
	if ( $id )
	{
		...
		foreach ( $cp_assignments as $cp_assignment )
		{
			...
			// Group Assignments by their Type
			$cp_assignments_options[ $cp_assignment['CATEGORY'] ][ $cp_assignment['ASSIGNMENT_ID'] ] = $cp_assignment['TITLE'];
		}
		...
		else
		{
			...
			else
			{
				$header .= '<td>' . SelectInput(
					( isset( $RET['ASSIGNMENT_ID'] ) ?
						$RET['ASSIGNMENT_ID'] :
						// Coming from the Assignments program, preselect Assignment in the list.
						( ! empty( $_REQUEST['assignment_id'] ) ? $_REQUEST['assignment_id'] : '' ) ),
					'tables[' . $id . '][ASSIGNMENT_ID]',
					_( 'Assignment' ),
					$cp_assignments_options,
					false,
					'group',
					! empty( $RET['ASSIGNMENT_ID'] )
				) . '</td>';
			}
...
function QuizGetCoursePeriodAssignments( $cp_id, $mp_id, $teacher_id, $no_quiz = false )
{
	...
	$assignments_RET = DBGet( "SELECT ga.ASSIGNMENT_ID,ga.TITLE,ga.DESCRIPTION,ga.POINTS,
		gat.TITLE AS CATEGORY
		FROM gradebook_assignments ga,gradebook_assignment_types gat
		WHERE (ga.COURSE_PERIOD_ID='" . (int) $cp_id . "'
		OR ga.COURSE_ID='" . (int) $course_id . "')
		AND gat.ASSIGNMENT_TYPE_ID=ga.ASSIGNMENT_TYPE_ID" . $mp_sql . $teacher_sql . $no_quiz_sql .
		" ORDER BY gat.SORT_ORDER IS NULL,gat.SORT_ORDER,gat.TITLE,ga.ASSIGNED_DATE,ga.TITLE" );
