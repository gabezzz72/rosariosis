...
{
	$subject_title = ParseMLField( DBGetOne( "SELECT cs.TITLE
...
function LessonPlanItemListOutput( $lesson_id, $items )
{
	...
	if ( ! $lesson_id )
	{
		...
		echo '<p>' . dgettext( 'Lesson_Plan', 'Part of the Lesson' ) . '</p>';

		// Do NOT use ListOutput to add lesson parts, causing display issues with tooltip & in responsive...
		DrawHeader(
			'<table class="width-100p cellpadding-5"><tr class="st"><td>' . $parts['TIME'] . '</td></tr>' .
			'<tr class="st"><td>' . $parts['TEACHER_ACTIVITY'] . '</td><td>' . $parts['LEARNER_ACTIVITY'] . '</td></tr>' .
			'<tr class="st"><td>' . $parts['ASSESSMENT'] . '</td><td>' . $parts['RESOURCES'] . '</td></tr></table>'
		);
...
if ( ! function_exists( 'LessonPlanSaveEntry' ) )
{
	...
	function LessonPlanSaveEntry( $entry_id, $cp_id, $columns )
	{
		...
		if ( ! empty( $columns['DATA'] ) )
		{
			...
			$columns['DATA'] = json_encode( $columns['DATA'] );

			if ( ! function_exists( 'db_query_prep' ) )
			{
				// Maintain backward compatibility.
				$columns['DATA'] = DBEscapeString( $columns['DATA'] );
			}
		}

		if ( ! empty( $columns['LENGTH_MINUTES'] ) )
		{
			// Fix SQL error invalid input syntax for type integer
			$columns['LENGTH_MINUTES'] = (int) $columns['LENGTH_MINUTES'];
		}
