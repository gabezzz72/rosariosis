...
if ( ! function_exists( 'ParentAgreementLoginForm' ) )
{
	...
	function ParentAgreementLoginForm()
	{
		...
		<form action="index.php?modfunc=first-login" method="POST" id="agreement-form" target="_top">
			<h3 class="center"><?php echo Config( 'PARENT_AGREEMENT_TITLE' ); ?></h3>
			<div style="max-width: 640px; max-height: 400px; padding: 0 11px; overflow-y: auto;">
		...
		<?php // Note: hidden input used by JS below ?>
		<input type="hidden" disabled id="logout_token"
			value="<?php echo function_exists( 'AttrEscape' ) ?
				AttrEscape( issetVal( $_SESSION['token'], '' ) ) :
				htmlspecialchars( issetVal( $_SESSION['token'], '' ), ENT_QUOTES ); ?>" />
		<script src="plugins/Parent_Agreement/js/LoginForm.js?v=10.1"></script>
